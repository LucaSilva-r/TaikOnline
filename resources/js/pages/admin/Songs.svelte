...
<script lang="ts">
    ...
    let { songs }: { songs: { data: Song[]; current_page: number; last_page: number; from: number; to: number; total: number; next_url: string | null; prev_url: string | null } } = $props();
</script>
...
<div class="flex flex-1 flex-col gap-4 p-4">
    <div>
        ...
        <p class="text-sm text-muted-foreground">Browse the imported song library and their metadata.</p>
    </div>
        ...
        <table class="w-full text-sm">
            <thead class="bg-muted/50 text-left">
                <tr>
                    ...
                    <th class="px-3 py-2 font-medium hidden md:table-cell">ID</th>
                    <th class="px-3 py-2 font-medium w-[120px]">Genre</th>
                    <th class="px-3 py-2 font-medium w-[140px] hidden lg:table-cell">Parts Set</th>
                    <th class="px-3 py-2 font-medium w-[80px] text-center hidden sm:table-cell">Tags</th>
                </tr>
            </thead>
            <tbody>
                {#each songs.data as song (song.id)}
                    <tr class="border-t">
                        <td class="px-3 py-2 font-mono text-xs text-muted-foreground">{song.song_no}</td>
                        <td class="px-3 py-2">
                            <div class="flex flex-col gap-0.5">
                                <span class="font-medium">{song.title}</span>
                                {#if song.title_en}
                                    <span class="text-xs text-muted-foreground">{song.title_en}</span>
                                {/if}
                            </div>
                        </td>
                        ...
                        <td class="px-3 py-2 hidden lg:table-cell">{song.partsset.label}</td>
                        <td class="px-3 py-2 text-center hidden sm:table-cell">
                            <div class="flex justify-center gap-1">
                                {#if song.has_extreme}
                                    <Badge variant="destructive" class="h-[20px] px-1.5 text-[10px]">EX</Badge>
                                {/if}
                                {#if song.has_papamama}
                                    <Badge variant="secondary" class="h-[20px] px-1.5 text-[10px]">PM</Badge>
                                {/if}
                            </div>
                        </td>
                    </tr>
...
{#if songs.last_page > 1}
    <div class="flex items-center justify-between px-4">
        ...
        <div class="flex items-center gap-2">
            ...
            {/if}
            <span class="inline-flex h-8 items-center justify-center px-3 text-sm text-muted-foreground">
