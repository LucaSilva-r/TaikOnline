...
static void *handle_client(void *argp) {
    ...
    char client_ip[INET6_ADDRSTRLEN];
    memcpy(client_ip, args->client_ip, sizeof(client_ip));
    free(args);
    ...
    fprintf(stderr, "TLS connected on %d from %s: %s\n", listen_port, client_ip, gnutls_session_get_desc(session));
...
int main(void) {
    ...
    for (;;) {
        ...
        for (int i = 0; i < count; i++) {
            ...
            int client_fd = accept(fds[i], (struct sockaddr *)&peer_addr, &peer_len);
            ...
            if (peer_addr.ss_family == AF_INET) {
                inet_ntop(AF_INET, &((struct sockaddr_in *)&peer_addr)->sin_addr, args->client_ip, sizeof(args->client_ip));
            } else if (peer_addr.ss_family == AF_INET6) {
                inet_ntop(AF_INET6, &((struct sockaddr_in6 *)&peer_addr)->sin6_addr, args->client_ip, sizeof(args->client_ip));
            } else {
